 ...
 * --------------------------------------------
 * - WebSocket: accepts original client on / (and any path)
 * - HTTP: serves the full game HTML
 ...
 *
 * This implements the recovered Free Dig protocol: login, world, players,
 * movement, chat, mining, block placement, and inventory updates.
 */
...
// Prefer the full client; fall back to older names used in earlier builds.
...
function parseChat(room, player, reader) {
  ...
  if (clipped.toLowerCase() === "/help") {
    send(
      player.ws,
      packetChat(
        "^7WASD moves. Mine blocks with the pickaxe. Place Grass/Dirt from the toolbar. ^2Chat is multiplayer."
      )
    );
    return;
  }
  ...
  switch (opcode) {
    case 4:
      ...
      send(
        ws,
        packetChat(
          "^2Shared Free Dig world. All players in this room see the same map."
        )
      );
      break;
...
// Accept the original client WebSocket on any path (including /).
...
httpServer.listen(PORT, HOST, () => {
  ...
  console.log(`Serving HTML: ${HTML_FILE} (exists=${fs.existsSync(HTML_FILE)})`);
});
