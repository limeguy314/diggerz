...
function makeWorld() {
  ...
  // This is the recovered original Free Dig map generation from the HTML.
  ...
  // Keep the original starting area stable and easy to read.
  for (let x = 4; x < 14; x++) {
    for (let y = 0; y < HEIGHT; y++) {
...
function packetPlayer(player) {
  ...
  w.guid([0, 0, 0, 0]);
  w.i32(0);
  // The recovered client applies this field as the player skin-tone multiplier.
  // 0.9 matches the original default tone instead of the over-bright 1.44 value.
  w.r8(0.9);
  w.r8(1);
  return w;
}
...
function packetMovement(player) {
  ...
  // Opcode 6 is parsed by the original client with a GUID prefix.
  w.guid(player.id);
  w.r8(player.x);
  w.r8(player.y);
  w.r8(0);       // velocity X
  w.r8(0);       // velocity Y
  w.r8(0);       // animation state/angle field
  w.r8(0);       // aim/rotation field
  w.u16(0);      // animation timer
  w.byte(0);
  w.byte(0);
  w.u16(0);      // facing/weapon offset X
  w.u16(0);      // facing/weapon offset Y
  return w;
}
...
function parseMovement(room, player, reader) {
  ...
  broadcast(room, packetMovement(player));
}
...
function handlePacket(ws, data) {
  ...
  switch (opcode) {
    case 4:
      ...
      send(ws, packetChat("^2Shared Free Dig world. All players in this room see the same map."));
      break;
    ...
    // Keep the reconstructed/local-service chat opcode accepted as well.
    case 12:
      parseChat(room, player, reader);
      break;
...
// Accept the original client connection at / (and also /ws) without changing the client protocol.
